...
void FugaGC_end(void* self) {
    ...
    FugaGCList_append(&gc->white, &gc->root);
    FugaGCList_append(&gc->white, &gc->black);
    FugaGCList_append(&gc->white, &gc->gray);
...
void FugaGC_mark(void* parent, void* child) {
    ...
    if (HEADER(child)->pass != gc->pass) {
        ...
        if (!HEADER(child)->root) {
            ...
            FugaGCList_push(&gc->gray, &HEADER(child)->list);
        }
    }
}
...
void* FugaGC_alloc(void* self, size_t size)
{
    ...
    FugaGCList_push(&gc->white, &header->list);
    gc->num_objects++;
    header->freeFn = NULL;
    header->markFn = NULL;
    header->size   = size;
...
void FugaGC_root(void* self) {
    ...
    FugaGCList_push(&HEADER(self)->gc->root, &HEADER(self)->list);
}
...
void FugaGC_unroot(void* self) {
    ...
    FugaGCList_unlink(&HEADER(self)->list);
    FugaGCList_push(&HEADER(self)->gc->white, &HEADER(self)->list);
}
