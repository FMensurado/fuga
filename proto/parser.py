...
def parse(code):
    ...
    object(number(1000))
    >>> parse('"xyz"lol')
    object(exp(string('xyz'), msg('lol')))
    >>> parse('()')
    object(object())
    >>> parse('(10)')
    object(object(number(10)))
    >>> parse('(foo, bar)')
    object(object(msg('foo'), msg('bar')))
    >>> parse('(foo = bar, baz)')
    object(object(slot(msg('foo'), '=', msg('bar')), msg('baz')))
    ...
    >>> parse('a * b * c')
    object(opexp([msg('a'), msg('b'), msg('c')], ['*', '*']))
    >>> parse('[a + b] * c')
    object(opexp([opexp([msg('a'), msg('b')], ['+']), msg('c')], ['*']))
    >>> parse("'a")
    object(quote(msg('a')))
    >>> parse("foo `bar")
    object(exp(msg('foo'), msg('get', object(quote(msg('bar'))))))
    """
...
def parse_part(toks):
    ...
    msgs = []
    while toks.peek() not in [tokens.EOF, tokens.SEPARATOR, tokens.RPAREN, tokens.RBRACKET, tokens.OPERATOR, tokens.EQUALS, tokens.BECOMES]:
...
def parse_root(toks):
    """root ::= bracketexp | exp"""
    if toks.peek() == tokens.LBRACKET:
        toks.next()
        exp = parse_exp(toks)
        if not toks.peek() == tokens.RBRACKET:
            raise SyntaxError, "expected RBRACKET, got %s" % toks.peek()
        toks.next()
        return exp
    else:
